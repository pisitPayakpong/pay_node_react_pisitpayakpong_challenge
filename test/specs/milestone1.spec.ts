...
import { checkTitle, fillCardDetails, fillPersonalDetails, FIVE_SECONDS, lessonSignUp, openRegistrationPane, serverRequest, stripeRequest, submitForm, test, TWENTY_SECONDS, TYPE_DELAY, VALID_CARD } from '../helpers';
...
test.describe('Using different test cards', () => {
  ...
  test('Should schedule a Lesson using a 3DS Card:3.22', async ({ page }) => {
    ...
    // Expect success
    await expect(page.locator('text=Woohoo! They are going to call you the shredder')).toBeVisible({ timeout: 10 * 1000 });
